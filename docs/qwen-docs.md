...
生成文書に誤りがあれば、対象項目だけを `--topic` で選び、`--repair-review review.json` を追加して再生成できます。1回の起動で投入するジョブは1件です。通信失敗や意味の不一致を理由に自動で再投入することはありません。

レビューJSONは `sources`（対象ファイルのpath・sha256の配列）、`claims`（選択した項目の元の説明）、`findings`（項目名から具体的な指摘への対応表）を持ちます。元のartifactの `metadata.json` と `claims.json` から作成してください。ソースのハッシュが変わっていたり、選択項目と指摘が一致しなかったりする場合は送信前に拒否します。レビューには公開してよい指摘だけを入れてください。

修正結果は別の出力ディレクトリへ保存し、元の回答を残します。再度ソースと照合し、直った項目だけを採用してください。形式検査の合格やモデルの自己評価は、内容の合格を意味しません。再試行回数と費用の上限は実行者が管理します。
