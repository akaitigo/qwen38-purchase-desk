...
`--thinking` は `enable_thinking=true` と `reasoning_effort=low` を送ります。思考量を指定しない試行では回答前に出力上限へ達したため、低い思考量を指定する構成にしています。ただし、各実行が上限内で回答する保証ではありません。

元の草稿に引きずられる場合は、`--repair-review` を付けず対象項目を再生成して比較します。`--thinking` で思考モードを明示的に有効化できますが、思考も出力上限4096トークンに含まれ、時間と料金が増える場合があります。途中で切れた回答は採用しません。確認済みの項目は固定し、変更項目とその影響範囲を再チェックしてください。
