...
2026-09-07に、GitHub ActionsからQwen3.8-27B-FP8を呼び、全10操作の文書を生成しました。初回は型と説明の誤りが残り、公開前の検査で停止しています。既存の `qwen-docs.yml` にある11項目の動作説明とは、別の検証です。新しい `api-docs.yml` のGPUジョブは、専用変数を有効にした場合だけ実行します。
...
ソースと照合して見つけた指摘は、`--findings` に操作ID別のJSONファイルを渡して修正へ加えられます。今回の指摘は `config/api-docs-feedback.json` に保存しました。これはモデル自身のレビュー結果とは区別して記録します。CIでは `RUNPOD_API_DOCS_REPAIR_FORMAT=patch`、`RUNPOD_API_DOCS_MAX_REPAIRS=3` などで切り替えられます。
...
生成済みの操作は作り直さず、未生成の操作と、必要な検査・修正・レビューを実行します。文章を確認して見つけた指摘は、操作IDごとにまとめた `config/api-docs-feedback.json` から渡せます。指摘はモデルの生成結果とは区別して保存します。CIのログには操作名、処理段階、完了したジョブIDを出力します。
...
最終的には、ソース更新を契機にCIからセルフホストしたQwenを呼び、全10操作のAPI文書を生成し、その文書をリポジトリへ追加できるところまで確認します。artifactが作られただけでは完了としません。以下は実測の完了条件で、現時点の達成結果ではありません。
