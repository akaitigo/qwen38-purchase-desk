...
The first slice covers three operations. Saved worker responses can be replayed;
...
def make_payload(op_id, entries):
    ...
    prompt = (
        ...
        'POSTにはapplication/jsonのrequestBody(schemaとexample)を含める。'
    ...
    return {'input': {'openai_route': '/v1/chat/completions', 'openai_input': {
        ...
        'messages': [{'role': 'system', 'content': prompt}, {'role': 'user', 'content':
            json.dumps({'operationId': op_id, 'method': method.upper(), 'path': path,
                        'securitySchemes': SECURITY, 'sources': list(evidence_catalog(entries).values())},
                       ensure_ascii=False)}],
...
def prepare(entries, out):
    ...
    manifest = {'source_files': source_identity(entries), 'payloads': payloads,
                'planned_generation_jobs': 3, 'jobs_submitted': 0,
...
def assemble(documents, entries):
    if len(documents) != len(OPERATIONS) or {d['operationId'] for d in documents} != set(OPERATIONS):
        raise ValueError('Exactly the three selected operations are required')
    spec = {'openapi': '3.1.0', 'info': {'title': 'Purchase Desk API — 認証と申請作成', 'version': 'preview'},
    ...
    for document in documents:
        ...
        if method == 'post':
            ...
            raise ValueError('GET session has no request body in this slice')
...
def render(spec, documents, entries, out, provenance):
    ...
    index = ['# Purchase Desk API', '',
             '認証と申請作成の3操作を対象にしたプレビューです。残りの操作はまだ含みません。', '',
    ...
    (out / 'authentication.md').write_text('\n'.join(authentication) + '\nログアウトとサーバー側の失効動作は、この3操作の動作検証には含みません。\n', encoding='utf-8')
...
def main():
    ...
    if args.command == 'prepare':
        print(json.dumps(prepare(entries, args.out), ensure_ascii=False, indent=2))
        return
    ...
    spec = assemble(documents, entries)
